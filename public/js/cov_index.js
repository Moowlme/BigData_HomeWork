$(document).ready(function () {
	...
	$.ajax({
		...
		success: function (data, status) {
			if (status == "success") {
				...
				var daydata = getchinaDayList(jsData);
				var daydetailsdata = getchinaDetailsDatalList(jsData);

				loadStackLineEcharts(daydata, daydetailsdata);

				// //新增确诊与新增治愈
				var addsdaydata = getchinaaddsStackDayList(jsData);
				var addsdaydetailsdata = getchinaaddsStackDataList(jsData);
	...
	$.ajax({
		...
		success: function (data, status) {
			if (status == "success") {
				...
				// //柱状图	
				setBarData(otherCountryData(jsData.foreign));
			} else {
				alert('error');
			}
		},
	...
	function getNewDayTotals(data) {
		...
		$('.set-nowconfirm').text(getdata.nowConfirm);
	...
	function getNewDayAdds(data) {
		...
		$('.set-cutconfirm').text(getdata.nowConfirm);
